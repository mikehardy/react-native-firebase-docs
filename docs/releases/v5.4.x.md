...
> *This semver policy change will begin with v5.5.0* The patch version will not allow breaking changes. Patch releases will contain bugfixes and necessary features required for Firebase SDK compatibility, but occasionally the underlying Firebase SDKs or React-Native ecosystem require breaking change in order for v5.x.x to stay relevant during the v5.x.x -> v6 transition. Adjust your `package.json` to use the `~` semver range specifier vs `^` to avoid unexpected breaking change.
...
- **Added Breaking change notice. v5.5.x will begin to allow breaking changes in minor releases**
...
v5.4.x supports iOS SDK version `5.19.0` and above; however it's recommended to update to `v5.20.2` and lock the versions specifically in your `Podfile`:
